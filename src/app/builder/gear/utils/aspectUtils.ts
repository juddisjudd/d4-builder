export const getAllowedAspectTypes = (slotType: string): string[] => {
  ...
  export const getWeaponTypes = (slotType: string, className: string): string[] => {
    ...
    switch (lowerClassName) {
      case 'barbarian':
        switch (lowerSlotType) {
          case 'bludgeoning weapon': return ['2h Mace'];
          case 'dual-wield weapon 1':
          case 'dual-wield weapon 2': return ['1h Sword', '1h Axe'];
          case 'slashing weapon': return ['2h Axe', '2h Sword'];
          default: return [];
        }
      case 'druid':
      ...
      case 'necromancer':
        switch (lowerSlotType) {
          case 'weapon': return ['Staff', '1h Sword', '1h Scythe', '2h Scythe', '2h Sword', '1h Dagger'];
          case 'offhand': return ['Shield'];
          default: return [];
        }
      case 'rogue':
        switch (lowerSlotType) {
          case 'ranged weapon': return ['Bow'];
          case 'dual-wield weapon 1':
          case 'dual-wield weapon 2': return ['1h Dagger', '1h Sword'];
          default: return [];
        }
      case 'sorcerer':
